var cheapest_pack_url="http://www.veriftarif.be/energievergelijker/gas_elektriciteit_energieleveranciers_maandelijks_overzicht_nl/";var cheapest_e_url="http://www.veriftarif.be/energievergelijker/elektriciteit_energieleveranciers_maandelijks_overzicht_nl/";var Decorators={pricing_type:function(v){if(v=='variable'){return"Variable"}else if(v=='fixed'){return"Fix"}else{return"Combination"}},pricing_type_description:function(v){if(v=='variable'){return"Prix variable"}else if(v=='fixed'){return"Prix fix"}else{return"Combination fix et variable"}},price_period:function(date_in_unixtime){var months=["janvier","février","mars","avril","mai","juin","juillet","août","septembre","octobre","novembre","decembre"];var date=new Date(date_in_unixtime);return months[date.getMonth()]+" "+date.getFullYear()},delta:function(a,b){return Math.round((a.price_excl_promo_year_raw-b.price_incl_promo_year_raw)/b.price_incl_promo_year_raw*100)},service_level:function(payment,invoicing,contact){if(payment=="prepaid"){return"Payé d'avance"}else if(payment=="free"){if(invoicing=="free"){if(contact=="free"){return"Service sans limitations"}else{return"Communication électronique seulement"}}else if(invoicing=="email"){if(contact=="free"){return"Facturation obligatoire par email"}else{return"Facturation obligatoire par courrier électronique et <br/>communication électronique uniquement"}}}else if(payment=="domi"){if(invoicing=="free"){if(contact=="free"){return"Prélèvement automatique obligatoire"}else{return"Prélèvement automatique obligatoire et<br />communication électronique seulement"}}else if(invoicing=="email"){if(contact=="free"){return"Prélèvement obligatoire et factures par email"}else{return"Service très limité"}}}else{return"Service sans limitations"}}}
...
keys.sort(function(a,b){return results[a].price_excl_promo_year_raw-results[b].price_excl_promo_year_raw});for(var i=keys.length-1;i>=0;i--){var result=results[keys[i]];if(result.supplier_name.toLowerCase()!=supplier){continue}
var safe_product_name=result.product_name.toLowerCase().replace(/[^A-Z0-9]+/ig,"-");body+='<tr>'+'<th scope="row">'+result.product_name+'</th>'+'<td data-title="Duurtijd">'+ProductSet.Decorators.contract_duration(result.product_contract_duration)+' an(s) </td>'+'<td data-title="Vast of variabel">'+Decorators.pricing_type(result.product_pricing_type)+'</td>'+'<td data-title="Groene energie">'+(result.product_green_percentage>0?"Oui":"Non")+'</td>'+'<td data-title="Dienstverlening">'+Decorators.service_level(result.product_service_level_payment,result.product_service_level_invoicing,result.product_service_level_contact)+'</td>'+'<td data-title="Klant worden"><a href="'+result.product_subscribe_url+'" target="_blank" id="table-signup-'+safe_product_name+'">Cliquez ici</a></td>'+'</tr>'}
...
keys.sort(function(a,b){return results[a].price_excl_promo_year_raw-results[b].price_excl_promo_year_raw});var cheapest="";for(var i=keys.length-1;i>=0;i--){var result=results[keys[i]];var korting="";if(result.supplier_name.toLowerCase()!=supplier){continue}
if(result.price_promo_year_raw>0){korting='<li><strong>Promotion de '+result.price_promo_year+'/an</strong>. Seulement disponible pour les nouveaux clients qui s\'enregistrent <strong>via Veriftarif</strong> pour '+ ProductSet.Decorators.date(result.price_period_end)+'</li>'}
html+='<div class="col-md-12 col-lg-12 details"><div class="col-md-6 col-lg-5 detail-info">'+'<h3>'+result.name+'</h3>'+'<ul>'+'<li>Dur&eacute;e: '+ProductSet.Decorators.contract_duration(result.product_contract_duration)+' an(s)</li>'+'<li>'+Decorators.pricing_type_description(result.product_pricing_type)+'</li>'+'<li>'+result.product_green_percentage+'% &eacute;nergie verte</li>'+'<li>'+Decorators.service_level(result.product_service_level)+'</li>'+korting+'</ul>'+'</div><div class="col-md-6 col-lg-6 col-lg-offset-1">'+'<h5>Ce tarif est pour</h5>'+'<p>'+result.product_tariff_description+'</p>'+'<p><strong class="dark">Cartes Tarifaires :</strong> <a href="/energie/tariefkaarten/2017-2/">Elektriciteit</a> / <a href="/energie/tariefkaarten/2017-2/">Gas</a></p>'+'<p><a href="'+result.product_subscribe_url+'" target="_blank" class="red">D&eacute;marrez votre simulation personalis&eacute;e</a></p>'+'</div>'+'<div class="col-sm-12"><p class="diff"><a href="'+(result.product_type=='pack'?cheapest_pack_url:cheapest_e_url)+'" target="_blank">En savoir plus sur ce tarif &gt;&gt;</a></p></div>'+'</div>'}
