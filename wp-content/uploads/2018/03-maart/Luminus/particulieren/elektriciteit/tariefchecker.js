...
function descriptions(results,supplier) {
  ...
  for (var i = keys.length - 1; i >= 0; i--) {
      ...
      var pdffile = result.name.replace(/\s/g, '-').toLowerCase();
      var duurtijd = ProductSet.Decorators.contract_duration(result.product_contract_duration);
      ...
      + '<p><strong class="dark">Tariefkaarten:</strong> <a href="/energie/tariefkaarten/'+ year +'/'+ month_nr +'-'+ month_full +'/'+ supplier_url +'/particulieren/elektriciteit/'+ pdffile +'">Elektriciteit</a> / <a href="/energie/tariefkaarten/'+ year +'/'+ month_nr +'-'+ month_full +'/'+ supplier_url +'/particulieren/aardgas/'+ pdffile +'">Gas</a></p>'
      + '<p><a href="' + result.product_subscribe_url + '" target="_blank" class="red">Word klant</a> of <a href="http://www.tariefchecker.be">bereken jouw persoonlijk tarief</a></p>'
      + '</div>'
