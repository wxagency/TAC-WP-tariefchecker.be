<?php
	class WpFastestCacheLogs{
		...
		public function getVia(){
			...
			if(isset($via[8]) && ($via[8]["function"] == "wp_set_comment_status") && ($via[2]["function"] == "deleteHomePageCache") && ($via[3]["function"] == "singleDeleteCache")){
				return false;
			}
			...
			}else if(isset($via[6]) && ($via[3]["function"] == "apply_filters" && $via[6]["function"] == "wpfc_clear_all_cache")){
				$arr["file"] = $via[6]["file"];
		...
		public function decodeVia($data){
			...
			}else if($data->function == "wpfc_clear_all_cache"){
				if(isset($data->file) && $data->file){
					return "<span>- ".$data->function."</span><br><span>".$data->file."</span>";
				}
			}
