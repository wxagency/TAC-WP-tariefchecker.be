<?php
	...
	class WpFastestCacheLazyLoad{
		...
		public function background_to_lazyload($content, $inline_scripts){
			...
			preg_match_all('/<(div|span|a)\s[^\>]+style\s*\=\s*[\"\'][^\"\']*(background\-image\s*\:\s*url\s*\(([^\)\(]+)\)\;?)[^\"\']*[\"\'][^\>]*>/i', $content, $matches, PREG_SET_ORDER);
			...
			if(count($matches) > 0){
				...
				*/
				foreach ($matches as $key => $div){
					...
					$tmp = preg_replace("/<([a-z]{1,4})\s/i", "<$1 data-wpfc-original-src='".$div[3]."' ", $tmp);
