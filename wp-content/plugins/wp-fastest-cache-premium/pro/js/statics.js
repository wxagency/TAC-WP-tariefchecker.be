var WpFcStatics = {
	...
	wpfc_get_servers: function(){
		...
		var servers = {
					   ...
					   "euw": [
							   ...
							   {"key": "uk", "time":0,"flag":"gb","location":"UK","color":"red", "url":"https://uk.wpfastestcache.com"},
		...
		if(self.country_code){
			if(america.search(self.country_code) > -1){
				...
				self.wpfc_get_server_time(servers.eum);
				...
				time_out = 5000;
			}else if(africa.search(self.country_code) > -1){
				self.wpfc_get_server_time(servers.euw);
				self.wpfc_get_server_time(servers.eum);
				self.wpfc_get_server_time(servers.eue);
			}else if(self.country_code == "XX"){
				self.wpfc_get_server_time(servers.naw);
				self.wpfc_get_server_time(servers.euw);
				self.wpfc_get_server_time(servers.as);
			}else{
				setTimeout(function(){
					alert("Unknown Country Code: " + self.country_code);
				}, 1000);
			}
		}else{
			self.wpfc_get_server_time(servers.naw);
		...
		jQuery.ajax({
			...
			error: function(x, t, m) {
				...
				if(t === "timeout" || t == "Internal Server Error") {
					...
					if(typeof window.wpfc_last_error_time == "undefined"){
						window.wpfc_last_error_time = Math.floor(Date.now() / 1000);
						reload = true;
					}else{
						if((Math.floor(Date.now() / 1000) - window.wpfc_last_error_time) > 30){
							alert(t);
						}
					}
				}
	...
	set_click_event_collapse_image_details: function(){
		jQuery("td.open-image-details").click(function(e){
			...
			var sub = jQuery("tr[post-id='" + jQuery(e.target).closest("tr[post-id]").attr("post-id") + "'][post-type='detail']");
