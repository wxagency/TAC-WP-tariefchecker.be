...
class elFinderEditorZohoOffice extends elFinderEditor
{
    ...
    public function init()
    {
        ...
        if (!empty($this->args['target'])) {
            ...
            if (($srcVol = $this->elfinder->getVolume($hash)) && ($file = $srcVol->file($hash))) {
                ...
                $full_file_path = base64_encode( $srcVol->getRootPath($hash) );
                $blogi = site_url() . "/?red_fm_connect_zoho=true&full_file_path=" . $full_file_path;

                //print_r( $srcVol->getRootPath($hash) ); exit();  //URL:protected    /ARGS:protected -> args -> target
                ...
                $data = array(
                    ...
                    'format' => $format,
                    'lang' => $lang,
                    //'saveurl' => elFinder::getConnectorUrl() . '?red_fm_connect_zoho=true&cmd=editor&name=ZohoOffice&method=save'.$cdata, 
                    'saveurl' => $blogi . '&cmd=editor&name=ZohoOffice&method=save'.$cdata, 
                );
                if ($cfile) {
                    $data['content'] = $cfile;
                }
    ...
    public function save()
    {
        if (isset($_POST) && ! empty($_POST['id'])) {
            ...
            $hash = $hash[1];
            $hash = "l1_" . $hash;
            update_option("red_fm_debug", $hash);
